...
from openrlhf.eval_src.Evaluator import get_evaluator
...
class Samples:
    ...
    def __init__(
        ...
        self.additional_reward = additional_reward or []
    ...
    def concat_samples(samples_list: List["Samples"], pad_token_id) -> "Samples":
        ...
        additional_reward = sum([s.additional_reward for s in samples_list], [])
...
class SamplesGenerator:
    ...
    @torch.no_grad()
    def generate_samples(self, all_prompts: List[str], answers, reflect_statuses, **generate_kwargs) -> List[Samples]:
        """
        ...
        rollout_samples = self._generate_vllm(all_prompts, answers, reflect_statuses, **generate_kwargs)
    ...
    def _generate_vllm(self, all_prompts: List[str], all_answers, all_reflect_statuses, **kwargs) -> List[Samples]:
        ...
        all_answers = sum([[answer] * n_samples_per_prompt for answer in all_answers], [])
        all_reflect_statuses = sum([[reflect_status] * n_samples_per_prompt for reflect_status in all_reflect_statuses], [])
        ...
        for i in range(len(all_outputs)):
            ...
            reflect_status = all_reflect_statuses[i]
        ...
        # calculate for reward shaping and reflect bonus
        all_queries = sum(
            [self.tokenizer.batch_decode(s.sequences, skip_special_tokens=False) for s in samples_list], []
        )
        
        # all_queries = [query.replace("<|endoftext|>", "").split("<|continue|>")[-1].split("<|reflect|>")[-1] for query in all_queries]
        all_queries = [query[-128:] for query in all_queries]
        
        assert len(samples_list) == len(all_queries) == len(all_answers) == len(all_reflect_statuses), \
            f"Length mismatch: {len(all_queries)} queries, {len(all_answers)} answers, {len(all_reflect_statuses)} reflect statuses"
        
        
        additional_reward = []
        for idx, data_tuple in enumerate(zip(all_queries, all_answers, all_reflect_statuses)):
            query, answer, reflect_status = data_tuple

            predict_ans = self.evaluator.extract_answer_from_model_completion(query)
            ret_score = self.evaluator.check_answers_equiv(answer, predict_ans)
            ...
            # Calculate reflect bonus
            reflect_bonus = 0
            #! -----------------------------
            if reflect_status != 0:
                # reflect_status: 1 means the seq correct then reflect
                # reflect_status: -1 means the seq incorrect then reflect
                contrastive_bonus = 0
                if reflect_status == -1:
                    if ret_score == 1:
                ...
                reflect_bonus = self.args.bonus_scale * contrastive_bonus

            ret_score = ret_score - 1 # reward shaping
            additional_reward.append(ret_score + reflect_bonus)
            
            samples_list[idx].additional_reward.append(ret_score + reflect_bonus)
...
class RemoteExperienceMaker(ABC):
    ...
    def make_experience(self, samples_list: List[Samples]) -> List[Experience]:
        ...
        additional_reward_list = [s.additional_reward for s in samples_list]
        attention_mask_list = [s.attention_mask for s in samples_list]
        action_mask_list = [s.action_mask for s in samples_list]

        # Get rewards from samples, such as rewards from agent's environment
        if samples_list[0].rewards:
            raise ValueError("Not supported to use samples with rewards, please use reward model instead.")
            ...
            r_refs = self.remote_reward_model.get_rewards.remote(queries_list, prompts_list)
        else:
        ...
        # Convert and add
        rewards_list = [
            sigmoid(reward) + torch.tensor(additional, dtype=reward.dtype)
            for reward, additional in zip(rewards_list, additional_reward_list)
        ]
