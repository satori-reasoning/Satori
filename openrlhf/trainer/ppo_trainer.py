...
class BasePPOTrainer(ABC):
    ...
    def evaluate(self, eval_dataloader, global_step, temperature=0.6, n_samples_per_prompt=1):
        ...
        with torch.no_grad():
            ...
            all_answers = []
            all_prob_accs = []
            all_reflect_statuses = []
            prompt_to_datasource = {}  # Dictionary to store mapping between prompts and their data sources

            for datasources, prompts, answers, prob_accs, reflect_statuses in eval_dataloader:
                all_prompts.extend(prompts)
                all_answers.extend(answers)
                all_prob_accs.extend(prob_accs)
                all_reflect_statuses.extend(reflect_statuses)
...
class PPOTrainer(BasePPOTrainer):
    ...
    def fit(
        ...
        for episode in range(episode, args.num_episodes):
            ...
            filtered_samples = []
            number_of_samples = 0
            for _, rand_prompts, answers, reflect_statuses in self.prompts_dataloader:
                ...
                print(f"Sampled examples: {sample0[0]}")
